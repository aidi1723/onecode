...
function mergeArrayByKey(target, source, keyField, depth, path) {
    ...
    for (const key of sourceByKey.keys()) {
        if (!seen.has(key)) {
            result.push(deepMerge({}, sourceByKey.get(key), depth + 1, path));
        }
    }
    return result;
}
function deepMerge(target, source, depth = 0, path = '') {
    const result = { ...target };
    for (const key of Object.keys(source)) {
        ...
        if (depth < MAX_MERGE_DEPTH &&
            ...
            result[key] = deepMerge(targetVal, sourceVal, depth + 1, currentPath);
        }
        else if (depth < MAX_MERGE_DEPTH &&
            Array.isArray(sourceVal) &&
            Array.isArray(targetVal) &&
            ARRAY_MERGE_KEYS[currentPath]) {
            result[key] = mergeArrayByKey(targetVal, sourceVal, ARRAY_MERGE_KEYS[currentPath], depth, currentPath);
        }
        else {
            ...
            for (const [key, value] of Object.entries(config.overrides)) {
                ...
                if (key === 'interface' &&
                    ...
                    for (const [field, fieldVal] of Object.entries(value)) {
                        if (!librechatDataProvider.INTERFACE_PERMISSION_FIELDS.has(field)) {
                            ...
                            for (const [sub, subVal] of Object.entries(fieldVal)) {
                                if (!librechatDataProvider.PERMISSION_SUB_KEYS.has(sub)) {
                                    uiOnly[sub] = subVal;
                                }
                            }
