...
function loadMemoryConfig(config) {
    ...
    if (hasValidAgent(config.agent) && ((_a = config.agent) === null || _a === void 0 ? void 0 : _a.enabled) == null) {
        winston.warn('[memory] Agent config detected without explicit `enabled: true`. Automatic memory extraction is now opt-in. Add `memory.agent.enabled: true` to keep automatic memory updates.');
    }
    const charLimit = (_b = librechatDataProvider.memorySchema.shape.charLimit.safeParse(config.charLimit).data) !== null && _b !== void 0 ? _b : 10000;
...
exports.isMemoryAgentEnabled = isMemoryAgentEnabled;
