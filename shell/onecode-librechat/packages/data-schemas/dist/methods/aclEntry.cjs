 ...
 */
const EMPTY_SUPERSETS = Object.freeze([]);
const supersetCache = new Map();
/**
 ...
 */
function permissionBitSupersets(requiredBits) {
    if (!Number.isInteger(requiredBits) ||
        requiredBits < 0 ||
        requiredBits > permissions.MAX_PERM_BITS ||
        (requiredBits & ~permissions.MAX_PERM_BITS) !== 0) {
    ...
    const supersets = [];
    for (let v = 0; v <= permissions.MAX_PERM_BITS; v++) {
...
function createAclEntryMethods(mongoose$1) {
    ...
    async function getSoleOwnedResourceIds(userObjectId, resourceTypes) {
        ...
        const deleteSupersets = permissionBitSupersets(librechatDataProvider.PermissionBits.DELETE);
        ...
        const otherOwners = await AclEntry.aggregate([
            {
                $match: {
                    ...
                    $or: [
                        ...
                        { principalType: { $ne: librechatDataProvider.PrincipalType.USER } },
                    ],
                },
            },
...
exports.permissionBitSupersets = permissionBitSupersets;
