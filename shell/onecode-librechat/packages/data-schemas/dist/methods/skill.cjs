...
var yaml = require('../utils/yaml.cjs');
...
const SKILL_NAME_MAX = librechatDataProvider.SKILL_NAME_MAX_LENGTH;
const SKILL_DESCRIPTION_MAX = librechatDataProvider.SKILL_DESCRIPTION_MAX_LENGTH;
const SKILL_DESCRIPTION_SHORT_THRESHOLD = librechatDataProvider.SKILL_DESCRIPTION_SHORT_THRESHOLD;
const SKILL_DISPLAY_TITLE_MAX = librechatDataProvider.SKILL_DISPLAY_TITLE_MAX_LENGTH;
const SKILL_BODY_MAX = librechatDataProvider.SKILL_BODY_MAX_LENGTH;
const SKILL_FILE_PATH_MAX = 500;
const SKILL_NAME_PATTERN = librechatDataProvider.SKILL_NAME_PATTERN;
...
function extractAlwaysApplyFromBody(body) {
    ...
    for (const line of block.split('\n')) {
        ...
        let value = yaml.stripYamlTrailingComment(line.slice(colon + 1).trim()).trim();
        if (value === '') {
            return { status: 'absent' };
        }
        if (value.length >= 2 &&
...
function createSkillMethods(mongoose, deps) {
    ...
    function buildSkillFilter(params) {
        ...
        if (params.search && params.search.length > 0) {
            const rx = new RegExp(string.escapeRegExp(params.search), 'i');
    ...
    function decodeCursor(cursor) {
        ...
        try {
            ...
            if (!decoded.updatedAt ||
                ...
                !objectId.isValidObjectIdString(decoded._id)) {
                return null;
            }
            return { updatedAt: new Date(decoded.updatedAt), _id: new ObjectId(decoded._id) };
        }
        catch (error) {
            winston.warn(`[skill.decodeCursor] Invalid cursor: ${error.message}`);
            return null;
        }
    }
    ...
    async function getSkillById(id) {
        var _a;
        if (typeof id === 'string' && !objectId.isValidObjectIdString(id)) {
            return null;
        }
        const Skill = mongoose.models.Skill;
    ...
    async function updateSkill(params) {
        ...
        if (!objectId.isValidObjectIdString(id)) {
    ...
    async function deleteSkill(id) {
        ...
        const objectId$1 = new ObjectId(id);
        const res = await Skill.deleteOne({ _id: objectId$1 });
        if (!res.deletedCount) {
            return { deleted: false };
        }
        await SkillFile.deleteMany({ skillId: objectId$1 });
        try {
            await deps.removeAllPermissions({ resourceType: librechatDataProvider.ResourceType.SKILL, resourceId: id });
        }
        catch (error) {
            winston.error(`[deleteSkill] Error removing permissions for ${id}:`, error);
        }
        return { deleted: true };
    }
    async function deleteUserSkills(userId) {
        ...
        const Skill = mongoose.models.Skill;
        const soleOwned = await deps.getSoleOwnedResourceIds(userObjectId, librechatDataProvider.ResourceType.SKILL);
        ...
        await Promise.allSettled(soleOwned.map((rid) => deps
            ...
            resourceType: librechatDataProvider.ResourceType.SKILL,
            resourceId: rid.toString(),
        })
            .catch((error) => winston.error(`[deleteUserSkills] Error removing permissions for ${rid}:`, error))));
    ...
    async function updateSkillFileCodeEnvIds(updates) {
         ...
         */
        const result = await tenantBulkWrite.tenantSafeBulkWrite(SkillFile, ops);
        if (result.modifiedCount < updates.length) {
            winston.warn(`[updateSkillFileCodeEnvIds] Persisted ${result.modifiedCount}/${updates.length} codeEnvRefs (matched ${result.matchedCount}). Subsequent primes for unmatched files will re-upload.`);
        }
...
exports.backfillDerivedFromFrontmatter = backfillDerivedFromFrontmatter;
exports.createSkillMethods = createSkillMethods;
exports.deriveStructuredFrontmatterFields = deriveStructuredFrontmatterFields;
exports.inferSkillFileCategory = inferSkillFileCategory;
exports.partitionIssues = partitionIssues;
exports.validateAlwaysApply = validateAlwaysApply;
...
exports.validateSkillDisplayTitle = validateSkillDisplayTitle;
