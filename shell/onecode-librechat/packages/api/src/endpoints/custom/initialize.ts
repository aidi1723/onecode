...
import { buildOneCodeModelKwargs } from './onecode';
...
export async function initializeCustom({
  ...
  if (options != null) {
    if (endpoint === 'OneCode') {
      options.llmConfig.modelKwargs = buildOneCodeModelKwargs(
        options.llmConfig.modelKwargs as Record<string, unknown> | undefined,
        req.body?.metadata,
      );
    }
