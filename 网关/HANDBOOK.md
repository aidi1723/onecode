...
Codex CLI `gpt-5.5`，同一任务 A/B 对照（详见上层仓库 `../reports/` 与 `docs/phase3-temporary-closeout-20260525.md` 的原始评测记录）：
...
# 2. 配置（token 用于隔离客户端，真实上游 key 只留在网关侧）
...
客户端的 `OPENAI_API_KEY` 填网关 token；真实上游 key 永远不离开网关进程，也不会回传给客户端。
