...
- `PYTHONPATH=src python3 -m onecode.cli benchmark --run --workspace-root <private-temp-path> --report <private-temp-path>`
...
- `PYTHONPATH=src python3 -m onecode.cli sandbox-smoke --workspace <private-temp-path> --report <private-temp-path>`
...
- `PYTHONPATH=src python3 -m onecode.cli sandbox-smoke --workspace <local-user-path> code/.onecode/sandbox-smoke --report <local-user-path> code/.onecode/sandbox-smoke/report.json`
  returned exit code `0` with `status: completed`, `exit_code: 0`,
  `stdout_tail: "True\n"`, and marker path
  `<private-development-workspace>/.onecode/sandbox-smoke/sandbox-smoke.txt`.
