...
```bash
mkdir -p <private-temp-path>
curl -sS -H 'authorization: Bearer dev-local-token' \
  'http://127.0.0.1:8080/v1/onecode/project/status?workspace=<private-temp-path>
curl -sS -H 'authorization: Bearer dev-local-token' -H 'content-type: application/json' \
  ...
  --data '{"workspace":"<private-temp-path>","git":true,"verifierPolicy":true}'
```
