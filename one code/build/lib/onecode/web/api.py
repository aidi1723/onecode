...
from onecode.kernel.approval_plans import (
    ...
    isolate_interrupted_approval,
    list_pending_approval_plans,
)
from onecode.kernel.checkpoint import FileLockBusy
...
def handle_onecode_plan_approval(plan_id: str, body: dict[str, Any]) -> tuple[dict[str, Any], int]:
    ...
    try:
        ...
        with ExitStack() as stack:
            try:
                stack.enter_context(approval_plan_lock(workspace, plan_id))
            except FileLockBusy:
                return error_payload("invalid_approval_plan", "approval_plan_in_progress"), 409
            return _handle_locked_plan_approval(workspace, plan_id, approved)
    except ValueError as exc:
        status = 409 if str(exc) in {
            "approval_plan_in_progress", "approval_plan_already_resolved", "approval_plan_state_conflict",
        } else 400
        return error_payload("invalid_approval_plan", str(exc)), status
    except OSError:
        return error_payload("approval_storage_unavailable", "approval storage unavailable"), 500


def _handle_locked_plan_approval(workspace: Path, plan_id: str, approved: bool) -> tuple[dict[str, Any], int]:
    if isolate_interrupted_approval(workspace, plan_id):
        return error_payload("approval_execution_interrupted", "approval execution interrupted; inspect evidence before creating a new plan"), 409
    ...
    try:
        ...
        result = {"run_id": None, "status": "halted", "reason": "approved_plan_execution_failed"}
        finalize_approval_plan(stored, "approved", result)
        return error_payload("approved_plan_execution_failed", "approved plan execution failed; inspect evidence before creating a new plan"), 500
    finalize_approval_plan(stored, "approved", result)
    return attach_shell_projection(result), 200
