...
@contextmanager
def approval_plan_lock(workspace: Path, plan_id: str) -> Iterator[None]:
    _validate_load_request(plan_id, DEFAULT_MAX_PLAN_AGE_SECONDS)
    lock_path = workspace.resolve() / ".onecode" / "approval-locks" / f"{plan_id}.lock"
    with file_lock(lock_path, blocking=False):
        yield


def isolate_interrupted_approval(workspace: Path, plan_id: str) -> bool:
    """Quarantine an abandoned claim while holding its approval lock; never replay it."""
    _validate_load_request(plan_id, DEFAULT_MAX_PLAN_AGE_SECONDS)
    root = workspace.resolve() / ".onecode"
    archived = root / "approval-plans" / f"{plan_id}.json"
    interrupted = root / "interrupted-plans" / f"{plan_id}.json"
    executing = root / "executing-plans" / f"{plan_id}.json"
    pending = root / "pending-plans" / f"{plan_id}.json"
    if archived.exists() or interrupted.exists():
        raise ValueError("approval_plan_already_resolved")
    if not executing.exists():
        return False
    if pending.exists():
        raise ValueError("approval_plan_state_conflict")
    interrupted.parent.mkdir(parents=True, exist_ok=True)
    executing.rename(interrupted)
    return True
...
def claim_approval_plan(
    ...
    *,
    ...
    interrupted = root / ".onecode" / "interrupted-plans" / f"{plan_id}.json"
    if archived.exists() or interrupted.exists():
        raise ValueError("approval_plan_already_resolved")
    if claimed.exists():
        raise ValueError("approval_plan_in_progress")
    try:
